...
const SERVER_COMPILE_TARGETS = [
  { id: 'native', label: 'Native Binary (server)', ext: '' },
  { id: 'linux-x64', label: 'Linux x64', ext: '' },
  { id: 'windows-x64', label: 'Windows x64', ext: '.exe' },
  { id: 'macos-arm64', label: 'macOS ARM64', ext: '' },
  { id: 'android-arm64', label: 'Android ARM64', ext: '.apk' },
  { id: 'raspberry-pi', label: 'Raspberry Pi', ext: '' },
  { id: 'ios', label: 'iOS (server only)', ext: '.ipa' }
];
...
function getCompileTargets() {
  return SERVER_COMPILE_TARGETS;
}
...
function mountFrontierRoutes(app) {
  ...
  app.get('/api/frontier/targets', (req, res) => {
    res.json({ ok: true, targets: getCompileTargets() });
  });
  ...
  app.post('/api/frontier/compile', (req, res) => {
    const { code, source, language, filename, target } = req.body || {};
    const src = code || source;
    if (!src || typeof src !== 'string') {
      return res.status(400).json({ ok: false, success: false, error: 'Missing code or source' });
    }
    const result = compileToNative(src, { language, filename, target });
    res.json({ ...result, success: !!result.ok, target: target || 'native' });
  });
}
